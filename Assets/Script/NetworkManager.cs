...
namespace Prototype.OverFlood
{
    public class NetworkManager : MonoBehaviour
    {
        LobbyManager lobby;
        [SerializeField]
        public ServerSettings advancedSettings;
        ...
        [SerializeField]
        Transform[] spawnZones;     // This is where we store the Transform data of the spawn points, used when spawning the players
        [SerializeField]
        GameObject playerObject;        // This is where we store the GameObject data of the player character; This can be transformed into an array, if you'd like to create a character-select

        [SerializeField]
        Button buttonPrefab;            // This is what will populate the server list (Since we can't easily create a button at run-time)
        [SerializeField]
        Transform buttonListHolder; // This is what will populate the server list (Since we can't easily create a button at run-time)
        ...
        public void StartServer()
        {
            ...
            else if (advancedSettings.username == "")
            {
                errorText.text = "Error: " + "Invalid Userame\n" + errorText.text;
                    ...
                    Network.InitializeServer(int.Parse(advancedSettings.maxPlayers) - 1, int.Parse(advancedSettings.serverPort), true);
